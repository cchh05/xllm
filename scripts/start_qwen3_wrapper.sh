#!/bin/bash
# Spike Day 5: launch xllm with LoRA wrapper compiled in (wrapper inactive, should equal baseline)
set -e
umask 022
rm -f core.*
...
echo "[INFO] launching xllm_multilora binary on npu:0, port 18001"
echo "[INFO] log: $LOG_DIR/qwen3_wrapper.log"
...
"$XLLM_BIN" \
  ...
  --enable_shm=true \
  --node_rank=0 > "$LOG_DIR/qwen3_wrapper.log" 2>&1 &
