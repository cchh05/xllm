...
bool DisaggPDScheduler::add_request(std::shared_ptr<Request>& request) {
  ...
  if (FLAGS_enable_flip_verbose_log) {
    LOG(INFO) << "FLIPDIAG add_request: request_id="
              << request->request_id()
  ...
  while (true) {
    ...
    if (FLAGS_enable_flip_verbose_log) {
      LOG(INFO) << "FLIPDIAG dispatch_add_new_requests_post: request_id="
                << requests[0]->request_id()
                << " cntl_failed=" << cntl.Failed()
                << " resps_size=" << resps.resps().size()
                << (resps.resps().empty()
                        ? std::string(" [empty]")
                        : " first_status=" + std::to_string(resps.resps()[0].status_code())
                          + " first_dp_rank=" + std::to_string(resps.resps()[0].dp_rank())
                          + " first_blocks=" + std::to_string(resps.resps()[0].blocks_ids_size()));
    }
    if (cntl.Failed()) {
