...
void ContinuousScheduler::handle_prefill_requests(
  ...
  while (!waiting_priority_queue->empty() && remaining_seq_budget > 0 &&
    ...
    for (auto& prefill_sequence : request->sequences()) {
      ...
      // CP prefill token alignment only applies in CP_PREFILL mode (cp>1). A
      // runtime flip to DP_DECODE makes the live cp_size 1, so derive it from
      // active_dp_size_ (dp>1 => DP_DECODE => cp==1, no alignment) rather than
      // the immutable startup options_.cp_size().
      const int32_t live_cp_size =
          active_dp_size_ > 1 ? 1 : options_.cp_size();
      ...
      if (any_empty) {
        ...
        if (!exceeded_backdoor) {
          if (FLAGS_enable_flip_verbose_log) {
            LOG_EVERY_N(INFO, 20)
                << "FLIPDIAG lopsided_defer: active_dp_size="
                << active_dp_size_ << " (waiting for both dp_ranks)";
          }
          return;
        }
        LOG(WARNING)
            << "FLIPDIAG lopsided_backdoor: active_dp_size="
