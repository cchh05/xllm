...
struct LoRAConfig {
  ...
  // ---- Adapter-affinity batching (2026-08-10, SGLang-aligned surface) ----
  //
  // Engine-side counterpart to the network gateway adapter-affinity
  // dispatch. When --max_loras_per_batch is less than --max_loras, the
  // ContinuousScheduler picks decode requests so each batch keeps its
  // distinct adapter count within this limit, avoiding mixed-adapter
  // batches that fall to the slow forward path.
  //
  // Flag surface follows SGLang: a single knob (max_loras_per_batch)
  // controls both "is affinity gating on" and "how strict". Set it equal
  // to max_loras to disable gating (strict-FIFO, byte-identical baseline).
  // Set it to 1 for pure fast-path (strict affinity). Values in between
  // trade tail latency for throughput.

  // Max distinct adapter_ids (base=0 counts as one slot too) allowed in a
  // single decode batch. Default equals max_loras -> gating is a no-op.
