...
void XServiceClient::heartbeat() {
  ...
  while (!exited_.load()) {
    ...
    // (set_block_manager_pool, called from LLMEngine::rebuild_block_manager_pool
    // during a runtime CP<->DP flip) may swap it under us at any point. Reading
    // the atomic once and using the local for the whole iteration keeps this
    // section race-free without needing a mutex.
    ...
    if (pool->options().enable_prefix_cache()) {
      pool->get_merged_kvcache_event(&event);
