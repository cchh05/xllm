...
ForwardOutput LLMEngine::step(std::vector<Batch>& batch) {
  ...
  if (FLAGS_enable_flip_verbose_log) {
    ...
    for (size_t i = 0; i < forward_inputs.size(); ++i) {
      if (i > 0) tokens_dbg += ",";
      tokens_dbg += std::to_string(
          forward_inputs[i].host_token_ids().numel());
    }
...
bool LLMEngine::switch_mode(int32_t target_mode) {
  ...
  if (failed_idx.empty()) {
    ...
    if (target_mode == static_cast<int32_t>(DualParallelArgs::Mode::CP_PREFILL)) {
      cp_size_ = paired;
      dp_size_ = 1;
    } else {
      cp_size_ = 1;
  ...
  for (size_t i = 0; i < worker_clients_.size(); ++i) {
    ...
    bool flipped = std::find(failed_idx.begin(), failed_idx.end(), i) ==
                   failed_idx.end();
    if (flipped) {
  ...
  if (!rb_futures.empty()) {
    ...
    for (const auto& r : rb_results) {
      if (!r.hasValue() || !r.value()) {
        LOG(ERROR)
            << "switch_mode: rollback fan-out also failed on a worker; "
...
void LLMEngine::rebuild_block_manager_pool(int32_t new_dp_size) {
  ...
  if (options_.host_blocks_factor() > 1.0 || options_.enable_kvcache_store()) {
    kv_cache_manager_ =
        std::make_unique<HierarchyBlockManagerPool>(opts, this, new_dp_size);
  } else {
    kv_cache_manager_ =
        std::make_unique<BlockManagerPool>(opts, new_dp_size);
  }
  ...
  LOG(INFO) << "rebuild_block_manager_pool: rebuilt for dp_size=" << new_dp_size;
}
