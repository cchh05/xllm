...
void ModeSwitchService::SwitchMode(
  ...
  DisaggPDScheduler* disagg =
      dynamic_cast<DisaggPDScheduler*>(scheduler_);
  ...
  if (scheduler_ != nullptr) {
    ...
    if (!drained) {
      LOG(WARNING) << "ModeSwitchService::SwitchMode timed out waiting for "
                      "scheduler to drain (" << drain_timeout_ms
                   << "ms); aborting flip";
      scheduler_->resume();
      response->set_ok(false);
      response->set_current_mode(prev);
  ...
  if (ok && disagg != nullptr) {
    if (!disagg->relink_after_flip()) {
      LOG(WARNING)
          << "ModeSwitchService: relink_after_flip reported failures; "
